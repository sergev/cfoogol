...
char token[MAXTOKEN];
char pending[MAXTOKEN];
char keytab[MAXTAB][MAXTOKEN];
char symtab[MAXTAB][MAXTOKEN];
char *usage = "usage: 'fc [-debug] infile [outfile]'";
...
char *defaultext(char *fname, char *ext, int force)
{
    ...
    while (*s)
        ++s;
    point = s;
    while (c = *s, s > result && c != '.')
        --s;
    if (c == '.') { /* some extension exists */
        point = s;
        if (!force)
            return result; /* don't worry about what it is */
    }
...
void get2(void)
{
    ...
    char *p = pending;
    while ((typ = type(c0 = getc(inf))) == WHITESPACE)
        if (c0 == '\n')
            ++linecount;
    if (c0 != EOF)
        *p++ = c0;
    if (typ == QUOTE) {
        ...
        *p++ = '"';
        ...
        while ((type(c = getc(inf)) == typ || (typ == LETTER && type(c) == NUMBER)) &&
               typ != RANDOM && c != EOF) {
            *p++ = c;
            typ  = type(c);
            if (++count == MAXTOKEN)
                error("Too long input token");
        }
        ungetc(c, inf);
    }
    *p = '\0';
...
int STATEMENT(void)
{
    ...
    return IO_STATEMENT() || WHILE_STATEMENT() || COND_STATEMENT() ||
           BLOCK() || /* the order is important here */
           ASSIGN_STATEMENT();
}
...
int IO_STATEMENT(void)
{
    ...
    return PRINTS_STATEMENT() || PRINTN_STATEMENT() || PRINT_STATEMENT();
}
